...
versão mais recente: ${isLatest}`)


console.log(banner.string)
console.log(color("\n\n Dark Itachi - BOT: CONECTADO ✔️\n\n"))
const conn = makeWASocket({ version,  logger: P({ level: 'silent' }),
printQRInTerminal: true,
browser: ['ꪶ𝐃𝐀𝐑𝐊 ❛☾︎ꨄ︎☽︎❜ 𝐈𝐓𝐀𝐂𝐇𝐈ꫂ', 'Anonimo', 'versão: ꝏ'], auth: state })
console.log(`
QRCode 📸👨‍💻`)
store.bind(conn.ev)
...
case 'menu':
sendBimg(from, `${logo}`, menu(p, botName, emoji, emoji2, status, dev, pushname, base, baileys, sender), "ꪶ𝐊𝐀𝐔𝐀𝐍ꫂ", [
...
exec(`webpmux -set exif ${addMetadata('ꪶ𝐃𝐀𝐑𝐊 ❛☾︎ꨄ︎☽︎❜ 𝐈𝐓𝐀𝐂𝐇𝐈ꫂ','ꪶ𝐊𝐀𝐔𝐀𝐍ꫂ')} ${rano} -o ${rano}`, async (error) => {
fs.unlinkSync(media)
reply(enviar.espere)
})
})
